        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: itemService.getMovedItems(),
              builder: (context, snapshot) {
                ...
                if (_searchQuery.isNotEmpty) {
                  docs = docs.where((doc) {
                    ...
                    final itemId = (data['id'] ?? data['itemId'] ?? data['ID'] ?? doc.id)
                        .toString()
                        .toLowerCase();
                    return itemId.contains(_searchQuery.toLowerCase());
                  }).toList();
                }
                            ...
                            if (direction == DismissDirection.startToEnd) {
                                  await Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (_) => ScanCodePage(
                                        expectedQr: itemId.toString(),
                                        reservedDocId: doc.id,
                                        isDispatchMode: true,
                                        title: 'Verify Dispatch - $itemId',
                                      ),
                                    ),
                                  );
                                  
                                  // ItemDataPage handles dispatch/deletion
                                  return false;
                            } else {
                              final confirmRemove = await showDialog<bool>(
                                ...
                                builder: (ctx) => _buildGlassDialog(
                                  context,
                                  title: 'Remove Item',
                                  message: 'Are you sure you want to remove item $itemId?',
                                  cancelRed: true,
                                ),
                              );

                              if (confirmRemove == true) {
                                await itemService.updateItemStatus(doc.id, 'cancelled');
                                return true;
                              }
                              return false;
                            }
                    ...
                    TextButton(
                      ...
                      style: TextButton.styleFrom(
                        ...
                        backgroundColor: cancelRed
                            ...
                            : confirmColor.withValues(alpha: 0.25),
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(12),
                        ),
                      ),
