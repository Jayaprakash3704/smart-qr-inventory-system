...
class ItemService {
  ...
  Future<Map<String, dynamic>> _getInventoryRules() async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data()!;
        final rules = {
          'max_rolls': (data['bin_capacity'] is int)
              ? data['bin_capacity']
              : (int.tryParse(data['bin_capacity'].toString()) ?? 10),
          'max_weight': (data['max_bin_weight'] is num)
              ? data['max_bin_weight'].toDouble()
              : 500.0,
          'max_bins': (data['max_bins'] is int)
              ? data['max_bins']
              : (int.tryParse(data['max_bins'].toString()) ?? 50),
        };
        debugPrint('DEBUG: Inventory Rules Loaded: $rules');
        return rules;
      }
    } catch (e) {
      debugPrint('DEBUG: Error reading config: $e');
    }
  ...
  Future<Map<String, int>?> getNextAvailableBin({
    ...
    for (int rId = minRackId; rId <= (minRackId + 50); rId++) {
      ...
      if (rackOccupancyCache != null &&
          rackOccupancyCache.containsKey(rId.toString())) {
      ...
      for (int bId = vBinId; bId <= maxBinsPerRack; bId++) {
        ...
        if (rackOccupancy.containsKey(binKey)) {
          ...
          currentWeight = (rackOccupancy[bId.toString()]?['weight'] ?? 0.0)
              .toDouble();
        } else if (rackOccupancy.containsKey('Bin $bId')) {
          currentCount = rackOccupancy['Bin $bId']?['count'] ?? 0;
          currentWeight = (rackOccupancy['Bin $bId']?['weight'] ?? 0.0)
              .toDouble();
        }
        ...
        if ((currentCount + count <= maxRolls) &&
            (currentWeight + requiredWeight <= maxBinWeight)) {
          debugPrint(
            'DEBUG: Found space in Rack $rId, Bin $bId (Occupancy: $currentCount rolls)',
          );
  ...
  Future<Map<String, Map<String, dynamic>>> _getRackOccupancy(
    String rackId,
  ) async {
    ...
    for (var doc in snap.docs) {
      ...
      occupancy[bin]!['weight'] =
          (occupancy[bin]!['weight'] as double) + weight;
    }
    return occupancy;
  }
  ...
  Future<DocumentSnapshot> getItem(String qr) {
    return _db.collection('items').doc(_getSafeId(qr)).get();
  }

  Future<DocumentSnapshot?> findItemByContent(String content) async {
    final raw = content.trim();
    if (raw.isEmpty) return null;

    try {
      final doc = await getItem(raw);
    ...
    try {
      ...
      if (decoded is Map<String, dynamic>) {
        final possibleId = decoded['id'] ?? decoded['itemId'] ?? decoded['ID'];
        if (possibleId != null) {
  ...
  // ================= RESERVED COLLECTION (Unified in 'items') =================

  Stream<QuerySnapshot> getReservedItems() {
    return _db
        .collection('items')
  ...
  Stream<QuerySnapshot> getMovedItems() {
    return _db
        .collection('items')
        .where('state', whereIn: ['picked', 'PICKED'])
        .snapshots();
  }

  Future<void> updateItemStatus(String docId, String newStatus) {
  ...
  Future<void> updateItemRollStatus(String docId, String newStatus) {
    return updateItemStatus(docId, newStatus);
  }

  Future<void> deleteReservedItemById(String docId) async {
    await _db.collection('items').doc(docId).delete();
  }

  // ================= ADD ITEM =================

  // ================= ADD ITEM (BATCH SMART ALLOC) =================

  /// Adds item(s) to Firestore.
  ...
  Future<String> addItem(
    ...
    if (binId != null && rackId != null) {
      ...
      final double currentWeight = (rackOccupancy[binKey]?['weight'] ?? 0.0)
          .toDouble();
      final double w1 =
          weightOverride ??
          (data['weight'] is num
              ? data['weight'].toDouble()
              : rules['default_weight']);

      if (currentCount >= rules['max_rolls']) {
        throw Exception(
          "No space in Bin $binId (Capacity: ${rules['max_rolls']} rolls reached)",
        );
      }
      if (currentWeight + w1 > rules['max_weight']) {
        throw Exception(
          "No space in Bin $binId (Weight limit: ${rules['max_weight']}kg reached)",
        );
      }
      if (binId > rules['max_bins'] && rackId == 1) {
        // Apply limit check
        throw Exception(
          "Invalid Bin: Rack $rackId only supports up to ${rules['max_bins']} bins.",
        );
      }
      // General limit check for any rack
      if (binId > rules['max_bins']) {
        throw Exception(
          "Bin $binId exceeds the maximum allowed bins per rack (${rules['max_bins']})",
        );
      }
    }

    // Prefetch starting IDs
    final nextIds = await _generateUniqueItemIds(count);
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (w == null)
        throw Exception(
          "Weight is missing for roll ${i + 1}. Every roll must have a weight.",
        );
      ...
      if (allocation != null) {
        ...
        throw Exception(
          "Warehouse Capacity Exhausted: No space found after Rack $startRack Bin $startBin",
        );
      }
      ...
      localOccupancy[localKey]!['count'] =
          (localOccupancy[localKey]!['count'] as int) + 1;
      localOccupancy[localKey]!['weight'] =
          (localOccupancy[localKey]!['weight'] as double) + w;

      // Prepare data
      final systemId = nextIds[i];
      final filteredData = Map<String, dynamic>.from(data);
      filteredData.removeWhere(
        (k, v) => v.toString().toLowerCase() == 'unknown',
      );
      ...
      final fullData = {
        'lot_number':
            filteredData['lot_number'] ??
            'LOT-GEN-${DateTime.now().millisecondsSinceEpoch.toString().substring(9)}',
        ...
        ...filteredData,
        'id': systemId,
        'originalQrId': data['id'] ?? data['itemId'] ?? qr.trim(),
        'bin': 'B$currentBin',
        'state': 'IN STOCK',
        'createdAt': now,
  ...
  Future<List<String>> _generateUniqueItemIds(int count) async {
    ...
    try {
      ...
      debugPrint('DEBUG ERROR in ID generation pre-fetch: $e');
    }

    return List.generate(count, (i) => 'YR-$year-${lastNum + i + 1}');
  }

  /// ✅ UPDATED parseItemData TO RETURN READABLE FIELDS
  Map<String, dynamic> parseItemData(String qr) {
  ...
  Future<void> deleteItem(String qr) {
